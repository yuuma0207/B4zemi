...
ボソンの交換に基づくポテンシャルの適切な導出は、今扱っている範疇を超えており、相対論的な場の量子論を扱う必要がある。
...
\begin{equation}
  ...
  \label{eq:3-59}
\end{equation}
電磁場が量子化されるとき、場の量子として光子が出現し、電荷は場の源(source of the field)になる。
\vskip\baselineskip
\begin{tcolorbox}[
    colback = white,
...
\begin{tcolorbox}[
    ...
    colframe = green!35!black,
    fonttitle = \bfseries]
  \begin{theorem}[中心力ポテンシャル下での運動]
    三次元極座標系におけるラプラシアンは
    \begin{equation}
      \laplacian = \frac{1}{r^2}\pdv{r}\qty(r^2\pdv{r})-\frac{\hat{\bm{l}}^2}{\hbar^2r^2}
    \end{equation}
    と変形できる。これは、$\hat{\bm{l}}^2=\hat{r}^2\hat{\bm{p}}^2 - (\hat{\bm{r}}\cdot\hat{\bm{p}})^2 + i\hbar(\hat{\bm{r}}\cdot\hat{\bm{p}})$
    となることからわかる。Schr\"{o}dinger方程式は、軌道角運動量量子数が$l$のとき、動径方向の波動関数を$R(r)$として、
    \begin{equation}
      \qty[-\frac{\hbar^2}{2m}\frac{1}{r^2}\pdv{r}\qty(r^2\pdv{r})+\frac{l(l+1)\hbar^2}{2mr^2} + V(r)]R(r)=ER(r)
    \end{equation}
    となる。自由粒子の場合$R(r)=rf(\rho=kr)$とおくと、$f(\rho)$は球Bessel微分方程式を満たす。
    \begin{equation}
      \dv[2]{f(\rho)}{\rho} + \frac{2}{\rho}\dv{f(\rho)}{\rho} + \qty{1-\frac{l(l+1)}{\rho^2}}f(\rho)=0
    \end{equation}
    この解は、$l$次の球Bessel関数$j_l(x)$である。
  \end{theorem}
\end{tcolorbox}
\vskip\baselineskip
...
2核子系であるため、スピン量子数$S$は$0$または$1$である。$S$の値を決めるために、核子に関与するスピンの配向性や偏極性を測定する必要がある。
...
\begin{tcolorbox}[
  ...
  \begin{theorem}[$t$行列について]
    ...
    \begin{equation}
      ...
      \label{eq:B64}
    \end{equation}
    式\eqref{eq:green}を用いると
    \begin{equation}
      \psi^+_{\bm{k}}=\qty(1+\sum_{n=1}^{\infty}(GV)^n)\phi_{\bm{k}}
    \end{equation}
    となることがわかり、
    \begin{equation}
      \bm{t}=V\qty(1+\sum_{n=1}^{\infty}(GV)^n) = V+VG\bm{t}
    \end{equation}
    となる。（ダイソン方程式と呼ばれるもの？）
    なお、これは（ファインマン？）ダイアグラムにより直感的に理解することができる。
  \end{theorem}
\end{tcolorbox}
\vskip\baselineskip
\newpage
